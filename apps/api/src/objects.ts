...
objects.post("/", async (context) => {
  ...
  try {
    await context.env.DB.prepare(
      ...
      .bind(id, objectKey, filename, contentType, file.size, createdAt)
      .run();
  } catch (error) {
...
objects.get("/:id", async (context) => {
  const row = await findObject(context.env.DB, context.req.param("id"));
  if (!row) {
...
objects.delete("/:id", async (context) => {
  ...
  await context.env.DB.prepare("DELETE FROM objects WHERE id = ?")
    .bind(row.id)
    .run();
...
async function findObject(database: D1Database, id: string) {
  return database
    .prepare(
      `SELECT id, object_key, filename, content_type, size, created_at
